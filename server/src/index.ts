...
function handleDashboardMessage(client: DashboardClient, msg: DashboardMessage): void {
  switch (msg.type) {
    case 'send_message':
      ...
      // Auto-recover: if mode is unexpectedly not 'participate' or 'lurk', reconnect rather than error
      if (client.mode !== 'participate') {
        console.warn(`send_message: unexpected mode '${client.mode}' for ${client.id}, auto-recovering to participate`);
