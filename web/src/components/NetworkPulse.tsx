...
import type { DashboardState, DashboardAction, PulseNode, PulseEdge, Particle } from '../types';
    ...
    const tick = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          dx = (dx / dist) * force;
          dy = (dy / dist) * force;
          if (!dragRef.current || dragRef.current.nodeId !== a.id) { a.vx += dx; a.vy += dy; }
          if (!dragRef.current || dragRef.current.nodeId !== b.id) { b.vx -= dx; b.vy -= dy; }
        }
      }
      ...
      for (const edge of edges) {
        ...
        if (!dragRef.current || dragRef.current.nodeId !== a.id) { a.vx += fx; a.vy += fy; }
        if (!dragRef.current || dragRef.current.nodeId !== b.id) { b.vx -= fx; b.vy -= fy; }
      }
